@php
    ...
    $hasAnyData = !empty($satkerAgreementAssessments) || !empty($poldaAgreementResults) || !empty($mabesAgreementResults);
@endphp
...
<div class="max-w-3xl space-y-8">
    @if ($hasAnyData)
        @foreach ($aspects as $aspect)
            @php
                $satkerContents = $satkerAgreementAssessments[$aspect->value] ?? null;
                $poldaResult = $poldaAgreementResults[$aspect->value] ?? null;
                $mabesResult = $mabesAgreementResults[$aspect->value] ?? null;
                        ...
                        <div class="mt-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                @if (($canEditAssessment ?? false) && $poldaResult->rekomendasi === \App\Enums\AssessmentResult::Revision)
                                    <flux:button variant="ghost" size="sm" icon="pencil-square" :href="route('grant-agreement.assessment', $grant)" wire:navigate>
        ...
        @endforeach
    @else
        <flux:text>{{ __('page.grant-detail.no-agreement-assessment-data') }}</flux:text>
    @endif
</div>
