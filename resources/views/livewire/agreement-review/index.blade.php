<div>
    ...
    <flux:table :paginate="$grants">
        <flux:table.columns>
            ...
            <flux:table.column>{{ __('page.agreement-review.column-donor') }}</flux:table.column>
            <flux:table.column>{{ __('page.agreement-review.column-value') }}</flux:table.column>
            <flux:table.column>{{ __('page.agreement-review.column-status') }}</flux:table.column>
            <flux:table.column align="end">{{ __('page.agreement-review.column-action') }}</flux:table.column>
        </flux:table.columns>
                        ...
                        <div class="flex justify-end gap-2">
                            ...
                            @if (in_array($grant->id, $reviewableIds))
                                <flux:button variant="primary" size="sm" wire:click="confirmStartReview({{ $grant->id }})">
                                    {{ __('page.agreement-review.start-review-button') }}
                                </flux:button>
                            @elseif (in_array($grant->id, $underReviewIds))
                                <flux:button variant="ghost" size="sm" icon="arrow-right" :href="route('agreement-review.review', $grant)" wire:navigate>
                                    {{ __('page.agreement-review.continue-review-button') }}
                                </flux:button>
                            @endif
                        </div>
                    </flux:table.cell>
            ...
            @empty
                <flux:table.row>
                    <flux:table.cell colspan="6" class="text-center">
                        {{ __('page.agreement-review.empty-state') }}
                    </flux:table.cell>
                </flux:table.row>
            @endforelse
        </flux:table.rows>
    ...
    <flux:modal wire:model.self="showStartReviewModal" class="min-w-[22rem]">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('page.agreement-review.start-review-confirm-title') }}</flux:heading>
                <flux:text class="mt-2">
                    {{ __('page.agreement-review.start-review-confirm-description') }}
                </flux:text>
            </div>
