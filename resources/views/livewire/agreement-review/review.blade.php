<div>
    <div class="mb-6 space-y-2">
        <flux:heading size="xl">{{ __('page.agreement-review.review-title') }}</flux:heading>
        <flux:text>
            ...
            <div wire:key="assessment-{{ $assessment->id }}" class="p-5 border rounded-lg border-zinc-200 dark:border-zinc-700 space-y-4">
                ...
                @if (isset($satkerAssessments[$assessment->aspek->value]))
                    <div class="space-y-3">
                        <flux:text class="font-medium text-zinc-500 dark:text-zinc-400 text-xs uppercase tracking-wide">
                            {{ __('page.agreement-review.satker-assessment-heading') }}
                        </flux:text>
                ...
                @if ($assessment->result?->keterangan)
                    <div class="p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg space-y-1">
                        <p class="font-medium text-sm text-amber-700 dark:text-amber-400">{{ __('page.agreement-review.result-remarks-label') }}</p>
                        <div class="prose prose-sm dark:prose-invert max-w-none">
                ...
                @if (! $assessment->result)
                    <div class="pt-1">
                        <flux:button variant="primary" size="sm" wire:click="openResultModal({{ $assessment->id }}, '{{ $assessment->aspek->label() }}')">
                            {{ __('page.agreement-review.submit-result-button') }}
                        </flux:button>
                    </div>
                @endif
            </div>
    ...
    <flux:modal wire:model.self="showResultModal" class="min-w-[36rem]">
        <form wire:submit="submitResult" class="space-y-6">
            <div>
                <flux:heading size="lg">{{ __('page.agreement-review.submit-result-button') }} Aspek {{ $selectedAspectLabel }}</flux:heading>
            </div>

            <flux:radio.group wire:model.live="result" :label="__('page.agreement-review.result-label')" variant="cards" class="flex-col">
                <flux:radio value="{{ \App\Enums\AssessmentResult::Fulfilled->value }}" :label="__('page.agreement-review.result-fulfilled')" />
                <flux:radio value="{{ \App\Enums\AssessmentResult::Revision->value }}" :label="__('page.agreement-review.result-revision')" />
                <flux:radio value="{{ \App\Enums\AssessmentResult::Rejected->value }}" :label="__('page.agreement-review.result-rejected')" />
            </flux:radio.group>
            ...
            @if ($result && $result !== \App\Enums\AssessmentResult::Fulfilled->value)
                <flux:editor
                    wire:model="remarks"
                    :label="__('page.agreement-review.result-remarks-label')"
