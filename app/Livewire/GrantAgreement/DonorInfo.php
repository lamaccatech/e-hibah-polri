...
class DonorInfo extends Component
{
    ...
    public function mount(Grant $grant): void
    {
        ...
        $this->grant = $grant;
        $this->isReadOnly = $grant->ada_usulan;
    ...
    public function render(): View
    {
        ...
        if (! $this->isReadOnly && $this->origin === 'DALAM NEGERI') {
        ...
        $categoryOptions = [];
        if (! $this->isReadOnly && $this->origin) {
        ...
        if (! $this->isReadOnly && mb_strlen($this->name) >= 2 && ! $this->selectedDonorId) {
        ...
        return view('livewire.grant-agreement.donor-info', [
